...
#stop selenium hub and notes to free up machine memory
...
--data @<(cat <<EOF
{
   "attachments":[
      {
         "fallback":"$features Web Regression - ($now): <$link|Full Report> \nTest Started: $started \nTest Completed: $finish \nDuration: $(($ELAPSED_TIME/60)) min $(($ELAPSED_TIME%60)) sec",
         "pretext":"$features Web Regression - ($now): <$link|Full Report> \nTest Started: $started \nTest Completed: $finish \nDuration: $(($ELAPSED_TIME/60)) min $(($ELAPSED_TIME%60)) sec",
         "color":"$color",
         "fields":[
            {
